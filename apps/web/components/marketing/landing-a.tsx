...
/**
 * Landing page A (Feedback #2 — D-17, F5.2): the Feedback #1 layout with new
 * words. The headline and tagline below are SET 1 of
 * docs/feedback-2-landing-copy.md, standing in until the tester picks; change
 * `HEADLINE` and `TAGLINE` once, here.
 */
const HEADLINE = ['ทริปหน้า', 'เริ่มวางได้เลย', 'วันนี้.'] as const;
const TAGLINE =
  'จะไปคนเดียวหรือไปกันหลายคน รู้แค่วัน รู้แค่ตั๋ว หรือยังไม่รู้อะไรเลย — เริ่มจากตรงไหนก็ได้ AI ช่วยร่างแพลนรายวันพร้อมงบ แล้วจดค่าใช้จ่ายจริงจบในที่เดียว';
const EYEBROW = 'วางแพลนทริปกับ ROVE';
    ...
    >
      ...
      <HeroCanvas
        ...
        /* Lines are broken by hand into a short stack (§3.1) — each `block` is
           ...
           promise and the period. The words are Feedback #2's (F5.1): the
           Feedback #1 stack "วางแพลน ไปเที่ยว / จดงบ / จบในที่เดียว." is
           set 2 in docs/feedback-2-landing-copy.md and comes back with one
           edit if the tester prefers it. */
        headline={
          <>
            {HEADLINE.map((line) => (
      ...
      <section className="bg-surface border-border border-y">
        <div className={`${SHELL_SECTION} relative py-16`}>
          ...
          {/* Two up from `sm`; on a phone the four steps read as a list with
              the number beside the title rather than four stacked blocks
              (F5.2). */}
          ...
          <Card className="relative overflow-hidden p-6 sm:p-8">
            <div className="relative flex flex-wrap items-center justify-between gap-6">
              <div className="max-w-md">
                ...
                <p className="t-h2">เลือกดอกไม้ประจำตัวได้ 20 แบบ</p>
                {/*
              ...
              {/* Twelve flowers, six to a row on a phone — the old wrap left
                  a lone pair on the last line at 375px (F5.2). */}
              <div className="grid w-full max-w-xs grid-cols-6 gap-1.5 sm:w-auto">
                {CHARACTERS.slice(0, 12).map((c) => (
                  <CharacterAvatar key={c.id} characterId={c.id} size="md" className="mx-auto" />
                ))}
              </div>
            </div>
          </Card>
