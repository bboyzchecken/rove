...
- [ ] A23.1 ต่อ `GET /users/me/points` เข้าหน้าเว็บ  ·  endpoint คืน `{balance, entries}` (30 แถวล่าสุดจาก `points_transactions` มี `type`/`amount`/`ref_id`/`ref_type`/`created_at` ครบ) อยู่ที่ [user.handler.go:99](apps/api/pkg/handlers/api/user.handler.go) แต่ `grep` ทั้ง `apps/web` ไม่เจอ caller — ProfileScreen โชว์แค่ยอดคงเหลือก้อนเดียว · **ต้องทำเพิ่ม:** ขยายเป็น cursor pagination (30 แถวไม่ใช่ประวัติ) + resolve `ref_id` เป็นชื่อทริป/พาร์ตเนอร์ ไม่ใช่ UUID เปล่า
- [ ] A23.2 `GET /users/me/audience` — สรุปคนตามรอยของเจ้าของบัญชีเอง  ·  ยอด `viewCount`/`cloneCount` มีอยู่แล้วแต่โผล่เฉพาะหน้า **สาธารณะ** `/u/[handle]` เจ้าของจึงต้องรู้ handle ตัวเองแล้วเปิดหน้า public ของตัวเองไปดู · endpoint นี้ให้ต่อทริป + รวม พร้อม breakdown ว่า clone ไหน award แต้มแล้วบ้าง (join `plan_clones` × `points_transactions.ref_id`)
- [ ] W23.1 หน้า/การ์ด "ประวัติแต้ม" ใน `/profile` — รูปแบบเดียวกับ `CreatorEarningsCard` ที่ทำ ledger ไว้ดีแล้ว (ต่อรายการ, ต้นทาง, สถานะ, ธง `estimated`) · earn สีบวก redeem สีลบ, กดที่รายการแล้วไปทริปต้นทางได้
- [ ] W23.2 การ์ด "คนตามรอยฉัน" บน `/home` หรือ `/profile` — กี่คนก๊อปแพลนเราไป ทริปไหนถูกตามรอยมากสุด ได้แต้มจากอันไหนไปแล้วเท่าไหร่
- [ ] X23.1 เทสต์: ledger ของ user A ต้องไม่โผล่ใน response ของ user B (เคส cross-user เหมือน §4.3)
...
- [ ] A24.1 `GET /public/stats` — คนที่วางแพลนกับ ROVE, จำนวนคนที่ตามรอย (`plan_clones`), จำนวนรีวิว + คะแนนเฉลี่ย, จำนวนแพลนสาธารณะ  ·  **cache ใน Redis** (แบบเดียวกับ `fx_cache`) — หน้า landing โหลดทุกครั้งที่มีคนเข้า ห้ามให้กลายเป็น COUNT(*) สี่ตัวต่อ request
- [ ] A24.2 `GET /public/reviews/recent` — รีวิวจากคนที่เที่ยวตามจริง (มี body, rating) พร้อมทริปต้นทาง · อ่านจาก `trip_reviews` ที่มีอยู่แล้ว (A11.5) เฉพาะทริปที่ `visibility='public'`
- [ ] W24.1 section สถิติบน landing page — วางระหว่าง steps กับ features · **ตัวเลขจริงเท่านั้น** ถ้ายังน้อยให้ซ่อน section ไม่ใช่ปัดขึ้นหรือใส่ตัวเลขปลอม (เหตุผลเดียวกับที่ `CreatorEarningsCard` return null เมื่อยังไม่มียอด)
- [ ] W24.2 การ์ดรีวิว "คนที่เที่ยวตามบอกว่า" บน landing + `/explore` — รีวิวคือสิ่งที่ทำให้ "ตามรอย" น่าเชื่อกว่ายอดวิว
- [ ] D24.1 ทบทวนกับที่ปรึกษากฎหมายว่า M23 เพียงพอกับข้อกำหนดเรื่อง "แต้มที่แลกเป็นมูลค่าได้" หรือยัง (ต่อจาก §16 บรรทัด 19 ส.ค. เรื่อง `/terms` เป็นฉบับร่าง)
